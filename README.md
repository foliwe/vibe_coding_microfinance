...
6. Bootstrap live test accounts with `npm run create:test-users`.
...
The next step after dependency install is connecting the apps to live Supabase data and replacing mock fixtures with authenticated queries/RPCs. Use `npm run create:test-users` to provision the default `admin@example.com` admin plus a branch manager, agent, member, assignment, and member accounts for end-to-end testing.
